 ...
 */
void MinPQ(int* pq[], int initCapacity) {
    pq[initCapacity + 1];
    n = 0;
}
...
/**
 * Removes and returns a smallest int on this priority queue.
 *
 * @return a largest key on this priority queue
 * @throws NoSuchElementException if this priority queue is empty
 */
int delMin(int* pq[], int length) {
    if (isEmpty(pq)) {
        printf("Priority queue underflow");
        return 0;
    }
...
int main() {
    ...
    while (count != 0) {
        scanf("%d\n", &index);
        if(index < range) {
            insert(minPQ, index, count);
        }
        else if(isEmpty(minPQ) != true) {
            printf("%d\n", delMin(minPQ, count));
        }
        count--;
    }
